...
the_data = cbind(the_data, do.call(cbind, lapply(list(gam,gbm,lasso,ridge), function(x) x[[1]])))
child_model_names = c('gam', 'gbm','lasso','ridge') #,'enet')
child_model_objs = setNames(lapply(list(gam,gbm,lasso,ridge), function(x) x[[2]]), child_model_names)
...
if(use_inla_country_fes) all_fixed_effects = paste(stacked_fixed_effects, paste(names(gaul_code), collapse = " + "), sep=" + ")
