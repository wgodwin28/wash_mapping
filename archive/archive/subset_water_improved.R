...
data_pt <- mutate(data_pt, wt_indic = (hh_size*pweight*w_imp)/pweight_total)
...
data_poly <- mutate(data_poly, wt_indic = (hh_size*pweight*w_imp)/pweight_total)
...
for (shp in unique(data_poly$shapefile)) {
  ...
  save(subset, file = paste0("J:/WORK/11_geospatial/wash/resampling/water/improved/subset_",shp,".RData"))
}

save(data_pt, file = "J:/WORK/11_geospatial/wash/resampling/water/improved/master_pt.RData")
save(data_poly, file = "J:/WORK/11_geospatial/wash/resampling/water/improved/master_poly.RData")
